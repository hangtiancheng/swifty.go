...
3. Use the structured output to tailor every question.
...
## Round 1 -- Fundamentals (20 questions)

Pick 20 items from the candidate's `frontend_stack` and `backend_stack`, weighted by `primary_role`.
...
## Round 2 -- Project Deep-Dive (1-3 projects)

For each of the candidate's most significant projects (1-3, selected based on `primary_role` and technical depth):
