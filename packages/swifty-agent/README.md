...
# @swifty-agent (frontend)

**The chat UI for the swifty_agent backend.**

A Lit + JSX single-page app with streaming replies, an AI-Ops panel, RAG file upload, markdown rendering, and browser observability — built on `@swifty.js/lit-jsx` and Tailwind CSS v4.
...
- **Browser monitoring** — `@swifty.js/sentry` with the Performance and Exposure plugins, reporting to the backend's `/api/log` → Prometheus bridge. A dev-only crash seeder exercises error reporting.
...
| Concern    | Choice                                                               |
| ---------- | -------------------------------------------------------------------- |
| Components | Lit 3 via `@swifty.js/lit-jsx` (JSX instead of `html\`\`` templates) |
| Build      | Vite 8                                                               |
...
```bash
pnpm install
pnpm --filter swifty-agent dev
```

The dev server proxies `/api` to the Go backend at `http://localhost:8123`, so run `swifty_agent` first (see [`swifty_agent/README.md`](../../swifty_agent/README.md)).

### Scripts

| Command                                | Description                                |
| -------------------------------------- | ------------------------------------------ |
| `pnpm --filter swifty-agent dev`       | Start the Vite dev server.                 |
| `pnpm --filter swifty-agent build`     | Type-check and produce a production build. |
| `pnpm --filter swifty-agent preview`   | Preview the production build.              |
| `pnpm --filter swifty-agent typecheck` | `tsc --noEmit`.                            |
| `pnpm --filter swifty-agent lint`      | `oxlint --fix`.                            |
| `pnpm --filter swifty-agent format`    | Prettier (with the Tailwind plugin).       |
