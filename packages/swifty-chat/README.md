...
# @swifty-chat (frontend)

**The web client for the swifty_chat IM backend.**
...
- **Cache dashboard** — connects to the backend's `swifty_cache` WebSocket dashboard at `/dashboard/ws` and can delete individual cached keys.
...
| Concern    | Choice                                                       |
| ---------- | ------------------------------------------------------------ |
| Components | Lit 3 via `@swifty.js/lit-jsx` (JSX)                         |
...
The app talks to the Go backend at `http://localhost:8000` / `ws://localhost:8000` (see [`src/config.ts`](./src/config.ts)); start `swifty_chat` first (see [`swifty_chat/README.md`](../../swifty_chat/README.md)).

### Scripts

| Command                                       | Description                                |
| --------------------------------------------- | ------------------------------------------ |
| `pnpm --filter swifty-chat dev`               | Start the Vite dev server.                 |
| `pnpm --filter swifty-chat build`             | Type-check and produce a production build. |
| `pnpm --filter swifty-chat preview`           | Preview the production build.              |
| `pnpm --filter swifty-chat typecheck`         | `tsc -b --noEmit`.                         |
| `pnpm --filter swifty-chat lint` / `lint:fix` | ESLint.                                    |
| `pnpm --filter swifty-chat format`            | Prettier (with the Tailwind plugin).       |
...
```
src/
├── main.ts                      # mounts <swifty-app>
├── app-root.ts                  # <swifty-app>: router + WS bootstrap
...
- `GET /dashboard/ws` — live `swifty_cache` statistics and key management.
