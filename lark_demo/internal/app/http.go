...
func (a *App) Engine() *lark_http.Application {
...
func (a *App) corsMiddleware(ctx *lark_http.Context, next func()) {
...
func (a *App) authMiddleware(ctx *lark_http.Context, next func()) {
...
func usernameFrom(ctx *lark_http.Context) string {
...
func (a *App) login(ctx *lark_http.Context, next func()) {
	...
	writeSuccess(ctx, lark_http.H{"token": token})
}

func (a *App) register(ctx *lark_http.Context, next func()) {
	...
	writeSuccess(ctx, lark_http.H{"token": token, "username": username})
}

func (a *App) getUserSessions(ctx *lark_http.Context, next func()) {
	username := usernameFrom(ctx)
	key := sessionsCacheKey(username)
	var sessions interface{}
	if a.cacheGetJSON(ctx.Request.Context(), key, &sessions) {
		writeSuccess(ctx, lark_http.H{"sessions": sessions})
		return
	}
	typedSessions := a.services.Sessions(ctx.Request.Context(), username)
	a.cacheSetJSON(ctx.Request.Context(), key, typedSessions)
	writeSuccess(ctx, lark_http.H{"sessions": typedSessions})
}

func (a *App) createSessionAndSendMessage(ctx *lark_http.Context, next func()) {
	...
	writeSuccess(ctx, lark_http.H{"session_id": sessionID, "answer": resp.Answer})
}

func (a *App) createSessionAndSendMessageStream(ctx *lark_http.Context, next func()) {
	var req questionRequest
	if err := ctx.BindJSON(&req); err != nil || !isValidQuestion(req) {
		writeCode(ctx, code.ParamsInvalid)
		return
...
func (a *App) sendMessageToSession(ctx *lark_http.Context, next func()) {
	var req questionRequest
	...
	writeSuccess(ctx, lark_http.H{"answer": resp.Answer})
}

func (a *App) sendMessageStreamToSession(ctx *lark_http.Context, next func()) {
	var req questionRequest
	if err := ctx.BindJSON(&req); err != nil || !isValidQuestionWithSession(req) {
		writeCode(ctx, code.ParamsInvalid)
		return
...
func (a *App) getChatHistoryList(ctx *lark_http.Context, next func()) {
	...
	if a.cacheGetJSON(ctx.Request.Context(), key, &history) {
		writeSuccess(ctx, lark_http.H{"history": history})
		return
	}
	...
	writeSuccess(ctx, lark_http.H{"history": typedHistory})
}

func (a *App) uploadFile(ctx *lark_http.Context, next func()) {
	username := usernameFrom(ctx)
	if username == "" {
		writeCode(ctx, code.TokenInvalid)
		return
	...
	writeSuccess(ctx, lark_http.H{"filepath": path})
}
...
func writeCode(ctx *lark_http.Context, result code.Code) {
	ctx.Status = http.StatusOK
	ctx.JSON(codeBody(result))
}

func writeSuccess(ctx *lark_http.Context, extra lark_http.H) {
