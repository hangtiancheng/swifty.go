...
| -------------------------------- | ----------------- | -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| [`swifty_http`](./swifty_http)   | HTTP framework    | [Koa.js](https://koajs.com)                        | Deferred response, Trie routing, middleware onion, WebSocket (RFC 6455), SSE — zero external dependencies                    |
| [`swifty_orm`](./swifty_orm)     | MongoDB ORM       | [Knex.js](https://knexjs.org)                      | Chainable query builder, transactions, grouped aggregation, streaming, auto-increment sequences                              |
| [`swifty_cache`](./swifty_cache) | Distributed cache | [groupcache](https://github.com/golang/groupcache) | Read-through + singleflight, write propagation, dual-level LRU, consistent hashing, etcd discovery, live dashboard           |
| [`swifty_rpc`](./swifty_rpc)     | RPC framework     | [grpc-go](https://github.com/grpc/grpc-go)         | grpc-go-style API over a custom TCP binary protocol, multiplexing, streaming, circuit breaker, rate limiting, load balancing |

The modules are independently versioned and importable. The only cross-module dependency is optional: `swifty_cache`'s real-time dashboard is served through `swifty_http`.
...
```bash
go get github.com/hangtiancheng/swifty.go/swifty_http
```
...
Full documentation: [`swifty_http/README.md`](./swifty_http/README.md)
...
Full documentation: [`swifty_orm/README.md`](./swifty_orm/README.md)
...
```bash
go get github.com/hangtiancheng/swifty.go/swifty_cache
```
...
Full documentation: [`swifty_cache/README.md`](./swifty_cache/README.md)
...
```bash
go get github.com/hangtiancheng/swifty.go/swifty_rpc
```
...
Full documentation: [`swifty_rpc/README.md`](./swifty_rpc/README.md)
...
├── swifty_http/         # HTTP framework        github.com/hangtiancheng/swifty.go/swifty_http
├── swifty_orm/          # MongoDB ORM           github.com/hangtiancheng/swifty.go/swifty_orm
├── swifty_cache/        # Distributed cache     github.com/hangtiancheng/swifty.go/swifty_cache
├── swifty_rpc/          # RPC framework         github.com/hangtiancheng/swifty.go/swifty_rpc
...
```bash
cd swifty_http && go test ./...
cd swifty_orm && go test ./...    # uses mongodb://localhost:27017 (MONGO_URI to override)
cd swifty_cache && go test ./...
cd swifty_rpc && go test ./...
```

## Versioning

Modules are released independently with per-module git tags (`swifty_http/v0.0.4`, `swifty_rpc/v0.0.5`, ...). `node tag.js` reads the version from `package.json` and tags every module in one shot; individual modules can be pinned with `node tag.js --cache=v0.1.0 --rpc=v0.2.0`.
