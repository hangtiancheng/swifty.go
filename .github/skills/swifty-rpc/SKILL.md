...
description: >
  ...
  github.com/hangtiancheng/swifty.go/swifty_rpc). Key exported symbols: rpc.Dial,
  ...
  imports github.com/hangtiancheng/swifty.go/swifty_rpc, uses rpc.NewServer /
  ...
  migrating net/rpc or grpc-go code to swifty_rpc. Skip for grpc-go itself,
  Connect-Go, Twirp, net/rpc code that does not import swifty_rpc, HTTP / SSE /
  WebSocket work (use swifty-http instead), or pure Protobuf schema questions
  unrelated to swifty_rpc dispatch.
---
...
14. The etcd key prefix `/github.com/hangtiancheng/swifty.go/swifty_rpc/services/` is hard-coded with no namespace or environment parameter. Multiple environments sharing one etcd cluster will collide.
...
- `swifty-cache` -- Use swifty-cache for in-process caching layers. swifty_rpc does not integrate a cache; combine the two when building services that need response memoisation on the client side.
- `swifty-http` -- Use swifty-http for HTTP servers, REST APIs, SSE, or WebSocket work. swifty_rpc is TCP-only with a custom binary protocol; the two are complementary but non-overlapping.
- `swifty-orm` -- Use swifty-orm for database access within RPC service handlers. Service methods registered with swifty_rpc often use swifty-orm internally to query persistence.
