...
name: swifty-orm
description: >
  Knex-style chainable MongoDB ORM for Go (swifty_orm module,
  github.com/hangtiancheng/swifty.go/swifty_orm). Use when writing or modifying Go
  code that calls swifty_orm.NewEngine, engine.Collection, engine.Model, the
  ...
  github.com/hangtiancheng/swifty.go/swifty_orm. Also use when the user asks about
  Knex-style query chaining in Go for MongoDB, BSON filter construction, the
  Engine lifecycle, or replacing knex+mysql usage with swifty_orm+mongodb. SKIP
  for GORM, sqlx, ent, raw mongo-driver code that does not import swifty_orm,
...
All exported types live directly in the `swifty_orm` package (flat layout, no
sub-packages).
...
`sc` parameter) into every swifty_orm method call inside the callback. If the
...
| `swifty_orm_test.go` | Unit and integration tests; requires a reachable MongoDB instance.                                                                                 |
...
- `swifty-cache`: Distributed cache framework. When building an application that
  caches MongoDB query results, combine swifty_orm for persistence with swifty_cache
  for distributed read-through caching and invalidation.
- `swifty-http`: HTTP server framework. Use swifty_http to expose REST endpoints
  that perform database operations via swifty_orm Engine instances. The Engine is
  typically initialized at server startup and injected into handlers.
- `swifty-rpc`: gRPC service framework. Use swifty_rpc when database-backed services
