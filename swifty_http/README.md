...
```go
app := swifty.New()       // bare Application
app := swifty.Default()   // with Logger + Recovery
...
Signature: `func(ctx *swifty.Context, next func())`

```go
func Timer() swifty.Middleware {
...
```
swifty_http/
  swifty.go        Application, compose(), ServeHTTP
