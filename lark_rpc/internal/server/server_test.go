...
func TestServerHandleAndShutdown(t *testing.T) {
	clientConn, serverConn := net.Pipe()
	client := transport.NewTCPConnection(clientConn)
...
func TestServerOptionsAndRegisterConcurrent(t *testing.T) {
	s, err := NewServer("127.0.0.1:0", WithServerCodec(codec.JSON))
	if err != nil {
		t.Fatalf("NewServer returned error: %v", err)
	}
	...
	if _, err := NewServer("127.0.0.1:0", WithServerCodec(codec.Type(99))); err == nil {
