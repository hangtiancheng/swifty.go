...
name: update-skill
description: >
  Regenerate the lark-go framework skill documentation by reading the latest source
  code and rewriting SKILL.md files for lark_cache, lark_http, lark_orm, and lark_rpc.
...
# Update Lark-Go Framework Skills
...
| ---------------- | ------------------------------------ |
| `lark_cache/`    | `.claude/skills/lark-cache/SKILL.md` |
| `lark_http/`     | `.claude/skills/lark-http/SKILL.md`  |
| `lark_orm/`      | `.claude/skills/lark-orm/SKILL.md`   |
| `lark_rpc/`      | `.claude/skills/lark-rpc/SKILL.md`   |
...
1. Read every `.go` file in the source directory (and subdirectories where they
   exist, such as `lark_rpc/pkg/`, `lark_rpc/internal/`, `lark_cache/pb/`).
