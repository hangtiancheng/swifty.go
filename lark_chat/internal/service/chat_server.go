...
func (s *Server) Start() {
	for {
		...
		case client := <-s.Login:
			...
			_ = client.Conn.WriteText("welcome to lark chat")
...
func NewClientInit(ws *lark_http.WSConn, clientId string) {
	client := &Client{
		Conn:     ws,
		Uuid:     clientId,
...
func clientWrite(c *Client) {
	for msg := range c.SendBack {
		if err := c.Conn.WriteMessage(lark_http.TextMessage, msg); err != nil {
