...
func Test_Node_Get(t *testing.T) {
	...
	sstWriter, err := NewSSTWriter("test_node_get.sst", opts)
	if err != nil {
		t.Fatal(err)
	}
	...
	node := NewNode(opts, "test_node_get.sst", sstReader, 0, 0, size, blockToFilter, index)
	for _, kv := range kvs {
		...
		if !bytes.Equal(v, kv.Value) {
			t.Errorf("key: %s, expect value: %s, got: %s", kv.Key, kv.Value, v)
		}
	}
	...
	if ok {
		t.Errorf("key: e, expect exist: false, got: true")
	}
}
...
func Test_Node_binarySearchIndex(t *testing.T) {
	tests := []struct {
		...
		{
			name: "key after the last index entry",
			index: []*Index{
				{Key: []byte("a")},
				{Key: []byte("b")},
			},
		...
		{
			name: "key between two index entries",
			index: []*Index{
				{Key: []byte("a")},
				{Key: []byte("b")},
				{Key: []byte("d")},
		...
		{
			name: "key before the first index entry",
			index: []*Index{
				{Key: []byte("b")},
				{Key: []byte("d")},
				{Key: []byte("e")},
		...
		{
			name: "key after every index entry",
			index: []*Index{
				{Key: []byte("b")},
				{Key: []byte("d")},
				{Key: []byte("e")},
