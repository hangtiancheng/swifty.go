...
// newTestOptions builds the sst layer options for tests. The directory is a
// temporary one owned by the test.
func newTestOptions(t *testing.T) *Options {
	t.Helper()

	bf, err := filter.NewBloomFilter(1024)
	if err != nil {
		t.Fatal(err)
	}
...
func Test_Block_ToBytes(t *testing.T) {
	...
	if got := block.ToBytes(); !bytes.Equal(got, expect.Bytes()) {
		t.Errorf("expect: %v, got: %v", expect.Bytes(), got)
	}
}
