...
func SplitTimerIDUnix(str string) (uint, int64, error) {
	...
	timerID, err := strconv.ParseInt(timerIDUnix[0], 10, 64)
	if err != nil {
		return 0, 0, fmt.Errorf("invalid timer id in %q: %w", str, err)
	}
	unix, err := strconv.ParseInt(timerIDUnix[1], 10, 64)
	if err != nil {
		return 0, 0, fmt.Errorf("invalid unix timestamp in %q: %w", str, err)
	}
