...
lark_orm.Error("connection lost")
lark_orm.Errorf("query failed: %v", err)

lark_orm.SetLevel(lark_orm.InfoLevel)   // all logs enabled (most verbose)
lark_orm.SetLevel(lark_orm.ErrorLevel)  // errors only
lark_orm.SetLevel(lark_orm.Disabled)    // all logs suppressed
```
...
- Missing collection before query execution: returns `lark_orm.ErrCollectionRequired`
...
| `lark_orm_test.go`   | Unit and integration tests                                                              |
