...
func newTestRouter() *router {
	...
	r.addRoute("GET", "/hello/b/c", func(c *Context) {})
	r.addRoute("GET", "/hi/:name", func(c *Context) {})
...
func TestGetRoute(t *testing.T) {
	r := newTestRouter()
	n, params := r.getRoute("GET", "/hello/lark")
	if n == nil || n.pattern != "/hello/:name" {
		t.Fatalf("matched route = %v, want /hello/:name", n)
	}
	if params["name"] != "lark" {
		t.Fatalf("name param = %q, want lark", params["name"])
	}
	...
	if n, params = r.getRoute("POST", "/hello/lark"); n != nil || params != nil {
...
func TestStaticRouteTakesPriorityOverWildcard(t *testing.T) {
	r := newRouter()
	var matched string
	r.addRoute("GET", "/files/*filepath", func(c *Context) { matched = "wildcard" })
	r.addRoute("GET", "/files/exact", func(c *Context) { matched = "exact" })
