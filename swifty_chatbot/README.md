...
**A RAG chatbot backend that exercises the whole swifty.go stack.**

An LLM chat service built on `swifty_http` (HTTP + SSE), `swifty_rpc` (a standalone AI service), `swifty_orm` (MongoDB), and a per-user vector store — with JWT auth, streaming replies, and document-grounded answers.

[![Go](https://img.shields.io/badge/Go-1.26%2B-00ADD8?logo=go&logoColor=white)](https://go.dev)
[![Module](https://img.shields.io/badge/module-swifty__chatbot-blue)](go.mod)
...
```text
     Client
       │  HTTP + SSE
       V
 ┌─────────────────┐        swifty_rpc (custom TCP binary protocol)
 │  HTTP Gateway   │  ──────────────────────────────────────────────>  ┌──────────────┐
 │  swifty_http    │        Complete / CompleteStream                  │  AIService   │
 └────────┬────────┘                                                   │ (RPC server) │
          │                                                            └──────┬───────┘
          │ swifty_orm                                                        │
          V                                                                   V
...
- **Streaming replies** — server-side streaming over `swifty_rpc` (`CompleteStream`) is piped out as Server-Sent Events by `swifty_http`.
...
- **Conversation persistence** — sessions and messages are stored in MongoDB through `swifty_orm`; history is restored into agent memory at startup by `LoadMessagesInto`.
...
```bash
go get github.com/hangtiancheng/swifty.go/swifty_chatbot
...
```json
{
  "app_name": "swifty-chatbot",
  "app_host": "0.0.0.0",
  "app_port": "8088",
  "rpc_addr": "127.0.0.1:19090",
  "mongo_uri": "mongodb://localhost:27017/",
  "mongo_database": "swifty_chatbot",
  "jwt_expire_hours": 8760,
  "jwt_issuer": "swifty-chatbot",
  "jwt_subject": "swifty-chatbot",
  "jwt_key": "change-me",
  "rag_docs_dir": "./docs",
  "ai_model_name": "qwen3",
...
The AI service exposes two methods, both dispatched reflectively by `swifty_rpc`:
...
└── internal/
    ...
    ├── app/                   # swifty_http routes, middleware, handlers
    ...
    └── store/                 # MongoDB persistence via swifty_orm
```
