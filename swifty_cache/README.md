...
- swifty_http: `github.com/hangtiancheng/swifty.go/swifty_http` (required only when using the dashboard feature)
...
func main() {
    ...
    srv, err := cache.NewServer(
        ...
        "swifty_cache",   // etcd service name (must be consistent across all nodes).
    ...
    picker, err := cache.NewClientPicker(
        ":9001",
        cache.WithServiceName("swifty_cache"),
    )
    if err != nil {
        log.Fatal(err)
    }
...
Alternatively, mount the handler on an existing `swifty_http` application:
...
`PeerPicker` uses consistent hashing to select the owner node for a given key. `Peer` is the RPC client abstraction for that node. swifty_cache ships with `ClientPicker` (etcd-based service discovery) and `Client` (gRPC implementation).
...
`StartDashboard` launches a standalone HTTP server at `addr` with a `/dashboard/ws` endpoint. It is safe to call multiple times; only the first invocation takes effect. `DashboardHandler` returns a middleware-compatible handler for integration with an existing `swifty_http` application.
...
| Dimension            | groupcache                   | swifty_cache                                    |
| -------------------- | ---------------------------- | ----------------------------------------------- |
...
swifty_cache preserves the core API philosophy and mental model of groupcache (`Group + Getter + PeerPicker + singleflight`) while targeting microservice scenarios that require write propagation, automatic service discovery, and operational visibility.
...
- `client_test.go`: success and error paths for Get/Set/Delete using a mock `pb.SwiftyCacheClient`.
